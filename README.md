...
В ChromaDB metadata сохраняются только:

- `created_at` - время начала задачи;
- `finished_at` - время окончания задачи.

`business_days` и `time_hours` рассчитываются на чтении из этих двух полей.
